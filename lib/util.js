...
util.logRequest = (req, message) => {
	// console.log(req.headers)
	const headers = {
		requestId: req.id,
		userAgent: req.header('user-agent'),
		acceptLanguage: req.header('accept-language'),
		xOrigLang: req.header('x-orig-lang'),
		xff: req.header('x-forwarded-for'),
	}
	if (headers.xff) {
		headers.clientIp = headers.xff.split(',')[0]
	}
	const definedHeaders = _.omitBy(headers, _.isUndefined)
	const formattedHeaders = _.map(definedHeaders, (value, key) => `${key}="${value}"`)
    const cookies = cookie.parse(req.header('cookie') || '')
	// console.log('cookies', cookies)
	util.log(message, ...formattedHeaders, `fsLanguage=${cookies.fslanguage}`)
}