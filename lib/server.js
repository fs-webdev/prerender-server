...
server._send = function(req, res) {
	...
	util.log(`action=sendResponse code=${req.prerender.statusCode} duration=${ms} url=${req.prerender.url} requests=${Object.keys(req.prerender.pageLoadInfo.entries)?.length}`);
};