...
server.onRequest = function(req, res) {
	...
	util.logRequest(req, `action=renderRequest url="${req.prerender.url}"`)
...
server._send = function(req, res) {
	...
	const {totalRequests, blockedRequests, cachedRequests, completedRequests} = Object.values(req.prerender.pageLoadInfo ? req.prerender.pageLoadInfo.entries || [] : []).reduce((stats, entry) => {
		// console.log('entry', entry, entry.responseParams, stats)
		stats.totalRequests++
		if (!entry.responseParams) stats.blockedRequests++
		else if (entry.responseParams && entry.fromMemoryCache || entry.responseParams.fromDiskCache) stats.cachedRequests++
		else stats.completedRequests++
		return stats
	}, {totalRequests: 0, blockedRequests: 0, cachedRequests: 0, completedRequests: 0})

	util.log(`action=sendResponse code=${req.prerender.statusCode} duration=${ms} url="${req.prerender.url}" requestId="${req.id}" totalRequests=${totalRequests} blockedRequests=${blockedRequests} cachedRequests=${cachedRequests} completedRequests=${completedRequests}`);
};