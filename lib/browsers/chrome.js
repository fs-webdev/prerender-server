...
const debug = require('debug')('prerender:chrome')
	...
	return new Promise((resolve, reject) => {
		...
		Promise.all([
			...
			Network.requestWillBeSent((params) => {
				...
				if (tab.prerender.logRequests || this.options.logRequests) util.log(`[${params.requestId}] + ${tab.prerender.numRequestsInFlight} ${params.request.url}`);
			...
			Network.loadingFinished(({requestId, timestamp, encodedDataLength}) => {
				debug('loadingFinished', requestId, timestamp, encodedDataLength, tab.prerender.requests)
				if (tab.prerender.requests[requestId]) {
					tab.prerender.numRequestsInFlight--;
					tab.prerender.lastRequestReceivedAt = new Date().getTime();

					if (tab.prerender.logRequests || this.options.logRequests) util.log(`[${requestId}] - ${tab.prerender.numRequestsInFlight} ${tab.prerender.requests[requestId]}`);
			...
			Network.loadingFailed((params) => {
				debug('loadingFailed', params, tab.prerender.requests)
				if (tab.prerender.requests[params.requestId]) {
					tab.prerender.numRequestsInFlight--;
					const blocked = params.blockedReason || params.canceled && params.errorText === 'net::ERR_ABORTED'
					if (tab.prerender.logRequests || this.options.logRequests) util.log(`[${params.requestId}] - ${tab.prerender.numRequestsInFlight} ${tab.prerender.requests[params.requestId]} ${blocked ? '[blocked]' : '[failed]'}`);
			...
			Network.requestServedFromCache((params) => {
				debug('requestServedFromCache', params, tab.prerender.requests)
				if (tab.prerender.requests[params.requestId]) {
					tab.prerender.numRequestsInFlight--;
					if (tab.prerender.logRequests || this.options.logRequests) util.log(`[${params.requestId}] - ${tab.prerender.numRequestsInFlight} ${tab.prerender.requests[params.requestId]} [cached]`);
					delete tab.prerender.requests[params.requestId];

					let entry = tab.prerender.pageLoadInfo.entries[params.requestId];
					if (entry) {
						entry.fromMemoryCache = true
					}
				}
			});
